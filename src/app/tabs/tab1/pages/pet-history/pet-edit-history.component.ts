...
export class PetEditHistoryComponent implements OnInit, OnDestroy {
  destroy$ = new Subject();
      ...
      switchMap(data =>
        combineLatest(
          [
            ...
            this.customerService.getPetHistory(data.customerId, data.petId, data.historyId).pipe(take(1))
          ]
        )
      ),
      takeUntil(this.destroy$)).subscribe(([pet, histories, currentHistory]) => {
